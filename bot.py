...
async def process_faq_about(callback: types.CallbackQuery):
    text = (
        ...
        "Усадьба <b>«Магия леса»</b> в самом сердце Беловежской пущи — это уникальное сочетание комфорта и дикой природы. Расположенная в уединенном месте на опушке леса, на большой ухоженной территории, она предлагает гостям возможность насладиться атмосферой одного из последних первозданных лесов Европы.\n\n"
...
async def process_faq_included(callback: types.CallbackQuery):
    text = (
        ...
        "• Микроволновка\n• Холодильник\n• Столовые приборы\n• Обеденный стол\n• Кофе/Чай\n\n"
        "✨ <b>В комплекте:</b>\n"
        "• Чай, постельное бельё, посуда, полотенца, тапочки, средства гигиены."
    )
    await callback.message.edit_text(text, reply_markup=get_back_to_faq_button(), parse_mode="HTML")
